...
# Get the user input and place in Entry text field
...
def calculate():
    ...
    try:
        res = parser.expr(entire_string).compile()
        result = eval(res)
        clear_all()
...
# Adding input field
display = Entry(root, bd=1, bg="White")
...
Button(root, text="C", command=lambda :clear_all()).grid(row=3, column=5)
